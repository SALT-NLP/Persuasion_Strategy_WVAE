...
from bertVAE import HierachyVAE
...
parser.add_argument('--generator-hidden-size', type=int, default=1024)
...
#parser.add_argument('--warm',default=False, type=bool, metavar='N')
...
parser.add_argument('--bert_encoder', action='store_true')
...
def main():
    ...
    #  ['Other', 'Concreteness', 'Commitment', 'Emotional', 'Identity', 'Impact', 'Scarcity']
    ...
    model = HierachyVAE(vocab_size, args.embedding_size, args.generator_hidden_size, args.generator_layers, args.z_size, n_labels, doc_labels, args.hard).cuda()
    ...
    #print(model.module.encoder.parameters())
    ...
    optimizer = AdamW(
        [
            {"params": model.encoder.parameters(), "lr": args.lrmain},
            {"params": logits_params, "lr": args.lrlast},
        ])
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model, args.data_path + 'model_last.pkl')
...
def create_generator_inputs(x, train = True):
    #print(x.shape)
    ...
    for i in range(0, x.shape[0]):
        ...
        for j in range(0, x.shape[1]):
            if x[i][j] != tokenizer._convert_token_to_id('[SEP]'):
                temp.append(x[i][j])
        prob.append(temp)
    ...
    for i in range(0, prob.shape[0]):
        for j in range(1, prob.shape[1]):
            if r[i, j] < args.word_dropout and prob[i, j] not in [tokenizer._convert_token_to_id('[SEP]'),tokenizer._convert_token_to_id('[PAD]')]:
                prob[i, j] = tokenizer._convert_token_to_id('[UNK]')
    return prob
...
def train(labeled_trainloader, unlabeled_trainloader, vocab, optimizer, model, criterion, epoch, n_labels, dist):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        prob = create_generator_inputs(xs, train = True)
        #non_trainable_idx = torch.where(mask2.view(batch_size * seq_num) == 1)
        ...
        #print(epoch, batch_idx, mse_loss)
        if batch_idx%100 == 0:
...
def validate(val_loader, model, criterion, epoch, n_labels, vocab):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (x, l, y, mask1, mask2, mask3, mask4, mid, sent_len, doc_len) in enumerate(val_loader):
            ...
            #prob = create_generator_inputs(x.view(batch_size*seq_num, seq_len), vocab, train = True)
            
            #_, kld_z, logits, q_y_softmax, t, strategy_embedding = model(x, prob, tau, mask1, mask2, args.hard, l_one_hot, doc_len = doc_len, sent_len = sent_len)
            
            logits, ___ = model.encode(x, sent_len = sent_len)
            #logits, _, __ = nn.parallel.data_parallel(model.encode, (x, sent_len))
...
class HierachyVAELoss(object):
    def __call__(self, logits, kld_z, q_y, q_y_softmax, t, mask1, mask2, mask3, mask4, x, l, y, l_one_hot, epoch, n_labels, dist, tsa_type = 'exp'):
        ...
        #mse_loss = F.mse_loss(t.view(-1), y.float())
        ...
        #print(logits.shape)
        xs = xs[:, 1:xs.shape[1]]
        #xs = xs.contiguous().view(-1)
        trainable_idx = torch.where(mask4.view(batch_size*seq_num) == 1)
        #print(logits.shape)
        #print(torch.max(logits[trainable_idx][:1], 2)[1])
        
        #print(xs[trainable_idx][:1])
        ...
        # p(y|x)
        #print(q_y[trainable_idx][:5])
        #print(ys[trainable_idx][:5])
        #print(epoch)
        ...
        # kld_y
        #g = Variable(torch.log(torch.Tensor([1.0/n_class])).cuda())
